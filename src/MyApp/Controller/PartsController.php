...
class PartsController {
    ...
    public function edit(Application $app, Request $request, $id) {
        ...
        $conn->update('public.parts', array('nome' => $nome, 'opcional' => $opcional), array('part_id' => $id));
        try {
    ...
    public function select(Application $app, $id) {
        ...
        $sql = "SELECT * FROM public.parts WHERE part_id = ?";
        $statement = $conn->executeQuery($sql, array($id));
    ...
    public function insert(Application $app, Request $request, $carro) {
        ...
        $opcional = $request->get('opcional');
        try {
            $conn->insert('public.parts', array('nome' => $nome, 'opcional' => $opcional, 'car_id' => $carro));
        } catch (\Exception $e) {
            return new JsonResponse(
                    array('message' => 'Não foi!'), 500
            );
    ...
    public function delete(Application $app, $id) {
        $conn = $app['db'];
        try {
            $conn->delete('public.parts', array('part_id' => $id));
        } catch (\Exception $e) {
            return new JsonResponse(
                    array('message' => 'Não foi!'), 500
            );
